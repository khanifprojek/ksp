...
class Mdb extends CI_Model 
{
    ...
    function formSimpan($kode)
    {
        ...
        $this->db->join('(select * from simpanan where kode_nasabah ='.$kode.' order by id desc limit 1) as simpanan2','simpanan2.kode_nasabah=nasabah.kode', 'left');
        $this->db->where('nasabah.kode', $kode);
//        $this->db->group_by('simpanan2.kode_nasabah');
        $this->db->group_by(['simpanan2.kode_nasabah', 'simpanan2.id']);
        $query = $this->db->get();
        return $query->result();
    }
    ...
    function getSaldoTerakhir($kode)
    {
        ...
        $this->db->from('nasabah');
        $this->db->join('(select * from simpanan where kode_nasabah ='.$kode.' order by id desc limit 1) as simpanan2','simpanan2.kode_nasabah=nasabah.kode');
